...
object ParticleUtils {
    ...
    fun spawnAnimationOnBlock(nameSpacedKey: Key, block: Block) {
        ScafallProvider.get().registries.particleAnimations[nameSpacedKey]?.spawn(block)
    }

    fun spawnAnimationOnLocation(nameSpacedKey: Key, location: Location) {
        ScafallProvider.get().registries.particleAnimations[nameSpacedKey]?.spawn(location)
    }

    fun spawnAnimationOnEntity(nameSpacedKey: Key, entity: Entity) {
        ScafallProvider.get().registries.particleAnimations[nameSpacedKey]?.spawn(entity)
    }

    fun spawnAnimationOnPlayer(nameSpacedKey: Key, player: Player, equipmentSlot: EquipmentSlot) {
        ScafallProvider.get().registries.particleAnimations[nameSpacedKey]?.spawn(player, equipmentSlot)
    }
