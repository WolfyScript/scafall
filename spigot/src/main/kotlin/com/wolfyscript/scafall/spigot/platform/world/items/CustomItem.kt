...
import com.wolfyscript.scafall.spigot.platform.world.items.meta.CustomItemTagMeta
import com.wolfyscript.scafall.spigot.platform.world.items.meta.Meta
import com.wolfyscript.scafall.spigot.platform.world.items.meta.MetaSettings
import com.wolfyscript.scafall.spigot.platform.world.items.reference.*
...
class CustomItem : AbstractItemBuilder<CustomItem>, Keyed {
    ...
    constructor(key: Key, itemStack: ItemStack) : this(
        key,
        StackReference(ScafallProvider.get(), 1, 1.0, BukkitStackIdentifier.ID, itemStack)
    )
        ...
        if (Streams.stream<String>(particlesNode.fieldNames())
            ...
            val playerSettings =
                particleContent.player
                    ?: ParticleContent.PlayerSettings(ScafallProvider.get()) //Create or get player settings
            particlesNode.fields().forEachRemaining { entry ->
                val loc = ParticleLocation.valueOf(entry.key)
                val value = entry.value
                if (value.isObject && value.has("effect")) {
                    val animation = ScafallProvider.get().registries.particleAnimations[objectMapper.convertValue(
                        value["effect"],
                        Key::class.java
                    )]
                    if (animation != null) {
    ...
    fun convertToReference(): StackReference {
        if (hasNamespacedKey()) {
            return StackReference(
                ScafallProvider.get(), reference.amount(), reference.weight(), CustomItemStackIdentifier(key()),
                itemStack
            )
        }
        return reference
    ...
    companion object {
        ...
        fun getByItemStack(itemStack: ItemStack?): CustomItem? {
            return ScafallProvider.get().registries.customItems.getByItemStack(itemStack)
        }
        ...
        private fun getKeyForData(type: Class<out CustomItemData>): Key {
            return ScafallProvider.get().registries.customItemData.getKey(type)
