...
class CustomDurabilityMeta : Meta(KEY) {
    init {
    ...
    override fun check(item: CustomItem, itemOther: ItemBuilder): Boolean {
        ...
        return if (meta0 && meta1) {
            when (option) {
                MetaSettings.Option.EXACT -> itemOther.customDurability == item.customDurability
                MetaSettings.Option.LOWER -> itemOther.customDurability < item.customDurability
                MetaSettings.Option.HIGHER -> itemOther.customDurability > item.customDurability
                else -> false
            }
        } else !meta0 && !meta1
    }
